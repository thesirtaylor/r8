...
@Injectable()
export class TheEntitiesService {
  private readonly group = 'rate-entity-group';
  private readonly stream = 'rate-entity-created';
  constructor(
    private readonly repository: TheEntityRepository,
    private readonly cache: RedisService,
    private readonly logger: AppLoggerService,
    private ds: DataSource,
  ) {}

  async search(dto: SearchTheEntityRequest): Promise<TheEntityListResponse> {
    const { q, type } = dto;
    ...
    const cached: TheEntityListResponse = await getCompression(
      this.cache,
      cachekey,
    );
  ...
  async create(dto: CreateTheEntityRequest) {
    try {
      ...
      const saved = await this.ds.transaction(async (manager) => {
        ...
        const entitydto = plainToInstance(CreateTheEntityDto, dto);
        const createEntity = manager.getRepository(TheEntity).create(entitydto);
        const saved = await manager.getRepository(TheEntity).save(createEntity);
        return saved;
      });
      if (!saved) {
        throw new RpcException({
  ...
  private makeIdempotencyKey(dto: CreateTheEntityRequest): string {
    return createHash('sha256').update(JSON.stringify(dto)).digest('hex');
  }
  private rate_data(data: TheEntity) {
    return {
      id: data.id,
      name: data.name,
      type: data.type,
